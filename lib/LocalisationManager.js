...
const parseToMoment = (date, format = null) => parseWithTz(date, Timezones.London, format)
const parseToDate = (date, format = null) => parseToMoment(date, format).toDate()
const getMoment = () => moment().tz(Timezones.London)
const now = () => getMoment().toDate()

const local = {
  getMoment: () => moment().tz(getTimezone()),
  parseToMoment: (date, format = null) => parseWithTz(date, getTimezone(), format),
}
